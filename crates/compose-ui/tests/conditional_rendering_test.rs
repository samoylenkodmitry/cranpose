...
/// Reproduces the exact bug from the demo app using with_key
#[composable]
fn with_key_conditional_app(counter: MutableState<i32>) {
    // This matches the demo app structure exactly:
    // 1. Read state outside with_key
    // 2. Use with_key with the value as the key
    // 3. Conditional inside with_key block
    let is_even = counter.get() % 2 == 0;
    compose_core::with_key(&is_even, || {
        if is_even {
            Text("if counter % 2 == 0", Modifier::padding(12.0));
        } else {
            Text("if counter % 2 != 0", Modifier::padding(12.0));
        }
    });

    // This should work (text inside closure)
    Column(Modifier::padding(16.0), ColumnSpec::default(), {
        let counter = counter.clone();
        move || {
            Text(
...
#[test]
fn test_with_key_conditional_bug() {
    // This test reproduces the ACTUAL bug from the demo app
    // The bug is that text inside with_key doesn't update
    ...
    eprintln!("\n========================================");
    eprintln!("FAILING TEST: with_key Conditional Bug");
    eprintln!("========================================\n");
    eprintln!("This reproduces the exact structure from the demo app:");
    eprintln!("  let is_even = counter.get() % 2 == 0;");
    eprintln!("  compose_core::with_key(&is_even, || {{");
    eprintln!("    if is_even {{ Text(\"if counter % 2 == 0\", ...) }}");
    eprintln!("    else {{ Text(\"if counter % 2 != 0\", ...) }}");
    eprintln!("  }});");
    eprintln!("");
    ...
    rule.set_content({
        let c = counter.clone();
        move || {
            with_key_conditional_app(c.clone());
        }
    })
    .expect("initial render succeeds");

    // Find the first TextNode (the conditional one)
    let mut conditional_text_id = None;
    for id in 0..20 {
        if let Ok(text) = rule
            .applier_mut()
            .with_node::<TextNode, _>(id, |node| node.text.clone())
        {
            if text.contains("if counter %") {
                eprintln!("Found conditional TextNode at id {}: '{}'", id, text);
                conditional_text_id = Some(id);
                break;
            }
        }
    }

    let text_id = conditional_text_id.expect("should find conditional TextNode");

    // Check initial text
    let initial_text = rule
        .applier_mut()
        .with_node::<TextNode, _>(text_id, |node| node.text.clone())
        .expect("should have text node");
    eprintln!("Initial text: '{}'", initial_text);
    assert_eq!(initial_text, "if counter % 2 == 0", "Initial text should be 'if counter % 2 == 0'");
    ...
    // THIS SHOULD FAIL - the text should change but it won't with the bug
    assert_eq!(
        text_after_odd, "if counter % 2 != 0",
        "BUG CAUGHT: Text should have changed to 'if counter % 2 != 0' but is still '{}'",
        text_after_odd
    );
    ...
    assert_eq!(
        text_after_even, "if counter % 2 == 0",
        "BUG CAUGHT: Text should be 'if counter % 2 == 0' again but is '{}'",
        text_after_even
    );

    eprintln!("\n✓ If this passes, the bug is fixed!");
    eprintln!("✗ If this fails, the bug is confirmed: with_key prevents text updates\n");
}
