...
use crate::word_boundaries::{find_word_start, find_word_end};
...
pub(crate) fn handle_key_event_impl(
    ...
    match event.key_code {
        ...
        KeyCode::ArrowLeft => {
            ...
            if event.modifiers.command_or_ctrl() && !event.modifiers.shift {
                ...
                let target = if !sel.collapsed() {
                    ...
                    text[..sel.start].char_indices().last().map(|(i, _)| i).unwrap_or(0)
                } else { 0 };
                state.edit(|buffer| buffer.place_cursor_before_char(target));
            }
            true
        ...
        KeyCode::ArrowRight => {
            ...
            if event.modifiers.command_or_ctrl() && !event.modifiers.shift {
                ...
                let target = if !sel.collapsed() {
                    ...
                    text[sel.end..].char_indices().nth(1).map(|(i, _)| sel.end + i).unwrap_or(text.len())
                } else { text.len() };
                state.edit(|buffer| buffer.place_cursor_before_char(target));
            }
            true
        ...
        KeyCode::ArrowUp => {
            ...
            let column = state.desired_column().unwrap_or_else(|| { state.set_desired_column(Some(col)); col });
            let target = if line_start == 0 {
                state.set_desired_column(None); 0
            } else {
        ...
        KeyCode::ArrowDown => {
            ...
            let target = if line_end >= text.len() {
                state.set_desired_column(None); text.len()
            } else {
