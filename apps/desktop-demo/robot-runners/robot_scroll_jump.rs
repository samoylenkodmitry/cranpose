...
fn main() {
    ...
    AppLauncher::new()
        ...
        .with_test_driver(|robot| {
            ...
            println!("  After fling: Item {} at Y={:.1}", after_fling_item, after_fling_y);
            ...
            println!("  BEFORE mouse down: Item {} at Y={:.1}", before_item, before_y);
            ...
            println!("  AFTER mouse down: Item {} at Y={:.1}", after_down_item, after_down_y);
            
            let item_jump = (after_down_item as i32 - before_item as i32).abs();
            if item_jump > 1 {
                println!("  ✗ JUMP DETECTED! Jumped {} items on mouse down!", item_jump);
            } else {
            ...
            println!("After fling:   Item {} (scrolled {} items)", 
                after_fling_item, after_fling_item - initial_item);
            println!("Before 2nd:    Item {}", before_item);
            println!("After 2nd down:Item {} (jumped {} items)", 
                after_down_item, item_jump);
            
            if item_jump > 1 {
                println!("\n✗ TEST FAILED: Content jumped {} items on second scroll!", item_jump);
                std::process::exit(1);
